...
当前版本为 **v0.2.1 本地试玩版**，支持本地模拟多人和机器人玩家。相较 v0.2，v0.2.1 已将主题词库从主逻辑中拆出，统一存放在根目录下的 `wordbanks/` 文件夹中，便于后续独立维护与扩展词库。
...
v0.2.1 是提交友好版本地试玩原型，重点展示完整玩法闭环、可运行性和词库可维护性。当前房间号仅用于展示完整交互流程，不连接真实服务器，不支持不同设备之间实时同步。
